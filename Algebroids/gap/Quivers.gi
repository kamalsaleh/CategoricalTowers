...
##
BindGlobal( "PARSE_PATH_CATEGORY_DATUM_FROM_STRING",
  function ( data )
    ...
    for mor in mors do
        ...
        if Length( g ) <> 3 then
            Error( "the generating morphism \"", mor, "\" has an unexpected format!\n" );
        fi;
...
InstallGlobalFunction( CreateCapQuiver,
  ...
  function ( data )
    ...
    if IsString( data ) then
        data := PARSE_PATH_CATEGORY_DATUM_FROM_STRING( data );
    fi;
...
##
InstallMethod( \.,
        "for a cap-quiver and a positive integer",
